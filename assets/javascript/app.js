...
function getInfoFromAPI(flag) {
    ...
    $.ajax({
        ...
        switch (flag) {
        case "0":
            func = "populateLocation(" + data + "," + flag + ");";
            break;
        case "1":
            func = "populateLocation(" + data + "," + flag + ");";
            break;
        case "2":
            func = "populateBreweryPlan(" + data + "," + flag + ");";
            break;
        }
        //console.log(func)
        eval(func)
    ...
    $("#searchBtn").on("click", function () {
        ...
        if (isOK == 1) {
            $("#mainContainer").load("./assets/html/userSelectPlan.html", function () {
                ...
                if (selSD != "" && selED != "") $(".cs-date").text(selSD + " - " + selED)
                getInfoFromAPI("2")
                getEvents()
...
function populateBreweryPlan(data) {
    ...
    for (var k = 0; k < data.length; k++) {
        ...
        sp1.attr("class", c2)
        sp1.text(data[k][key])
        sp1.attr("title", "<b>Brewery Type:</b> <i>" + ucwords(data[k]["brewery_type"]) + "</i><br/>" + "<b>Address: </b><i>" + data[k]["street"] + ", " + data[k]["city"] + ", " + data[k]["state"] + "</i><br/><br/>" + "Click on the brewery name to see the website.")
        a.append(sp1)
        sp.append(inp)
        sp.append(a)
        $("#" + id).append(sp)
